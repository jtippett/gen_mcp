...
* `:task_store` - Optional task store module and options for durable task tracking The default value is `nil`.
...
>
> When authoring extensions, ensure your resource and prompt prefixes are unique
> to avoid conflicts with other extensions or the user's own configuration.